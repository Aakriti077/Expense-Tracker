...
// Fetch total income
$sql = "SELECT SUM(amount) AS total_income FROM income WHERE user_id = {$_SESSION['user_id']}";
$result = mysqli_query($conn, $sql);
$total_income = ($result && mysqli_num_rows($result) > 0) ? mysqli_fetch_assoc($result)['total_income'] : 0;

// Fetch total expenses
$sql = "SELECT SUM(amount) AS total_expense FROM expenses Where user_id = {$_SESSION['user_id']}";
$result = mysqli_query($conn, $sql);
...
// Log error message before sending JSON response
